...
void signalModelFormat(const char* filename, TString cicGenIso, TString kpl, TString theMass, TString cat) {
  ...
  if( fileOrig ) {
    fileOrig->cd();
    TString theTree = TString::Format("RSGravToGG_kMpl_"+kpl+"_M_"+theMass+"_TuneCUEP8M1_13TeV_"+cat);
  ...
  // New file
  TString theNewFile = TString::Format("/tmp/soffi/FormSigMod_"+cicGenIso+"_kpl"+kpl+"_M"+theMass+"_cat"+cat+".root");
